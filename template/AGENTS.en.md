...
4. **Annotate the source (single marker — replace, never append)**: every item added or revised in `plan.md` by this `fix-plan` carries exactly **one** source marker. When you revise an item that already has a marker, **rewrite that marker; do not append another citation**. Keep at most two references — the original source and the latest revision (`fix-plan_<desc>.md` or its archived `.cairn/archive/fix-plan_desc-YYYYMMDD.md`) — each with a date (at least `YYYY-MM-DD`; add `HH:mm TZ` when known). The full revision chain already lives in `.cairn/archive/` and git history, so a plan item must never accumulate a growing list of `source / revised-source` citations; if a marker has already grown past two references, collapse it to original + latest as part of this edit.
   - Recommended format: `(source: <initial>, <date>; last revised: <latest>, <date>)`; write a single entry when the two are the same.
...
- **`fix-plan_<desc>.md`**: extends or revises the plan's scope (new features, new modules, new acceptance criteria). When handling a plan modification request, first create `fix-plan_<desc>.md`, then modify `plan.md` from that file, annotate the changed plan items with a single consolidated source/date marker (replace in place, never append — see Lifecycle step 4), archive the `fix-plan` first, and only then decide how to update or add `TODO.md` entries from the revised plan.
...
- **Whenever a new plan or feedback appears**: if it is a plan modification, first create and confirm `fix-plan_<desc>.md`, update `plan.md` directly with a single source/date marker per item (replace, never append), archive the `fix-plan`, and only then derive or adjust `TODO.md` from the revised `plan.md`; if it is defect, test, or review feedback, first create or update `fix_<desc>.md`, then map it to TODO; if completed TODOs are affected, inspect the current TODO section or relevant `.cairn/archive/done-YYYYMMDD.md` before deciding whether old items become `[!]` / `[~]`, a re-review TODO is needed, or an archive note should be added.
